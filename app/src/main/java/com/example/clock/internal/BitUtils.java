...
public final class BitUtils {
    ...
    public static void checkArgument(boolean cond, String s) throws IllegalArgumentException {
        if (!cond) {
    ...
    public static boolean maskedEquals(byte[] a, byte[] b, @Nullable byte[] mask) {
        ...
        checkArgument(a.length == b.length, "Inputs must be of same size");
        if (mask == null) return Arrays.equals(a, b);
        checkArgument(a.length == mask.length, "Mask must be of same size as inputs");
